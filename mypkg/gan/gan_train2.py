...
class MyGAN2():
    def __init__(self, Generator, Discriminator, d, train_data, save_dir, 
        ...
        self.netG = Generator(self.d, self.nz).to(self.params_gan.device);
        self.netD = Discriminator(self.d).to(self.params_gan.device);
    ...
    def train(self, num_epochs, data_test=None, save_snapshot=True):
        ...
        for epoch in pbar:
            ...
            for i, data in enumerate(self.train_data_loader, 0):
                X, realY = data
                X, realY = X.to(self.params_gan.device), realY.to(self.params_gan.device)
                ...
                errG1 = self.criterion(output, label)
                errG2 = self.criteriony(lin.view(-1), realY.view(-1))/5
                errG = errG1 + errG2
                ...
                if self.verbose:
                    if (iters+1) % 50 == 0:
                        ...
                        if data_test is not None:
                            ...
                            self.netG.eval()
                            with torch.no_grad():
                                tY_hat0, _ = self.netG(tX, tZ)
                                tY_hat0 = tY_hat0.detach().numpy().reshape(-1);
                                test_err = np.sqrt(np.mean((tY_hat0 - data_test.Y1)**2))
                                v, _ = ss.pearsonr(data_test.Y1, tY_hat0)
                            self.netG.train()
                ...
                self.G1_losses.append(errG1.item())
                self.G2_losses.append(errG2.item())
        ...
        self.netG.eval()
        self.netD.eval()
        self.G1_losses = np.array(self.G1_losses)
        self.G2_losses = np.array(self.G2_losses)
        self.D_losses = np.array(self.D_losses)
        self.test_errs = np.array(self.test_errs)
    ...
    def get_opt_model(self, ws=None, m_tol=0.05, sd_tol=0.05):
        ...
        assert (self.D_losses is not None) and (self.G1_losses is not None) and (len(all_models) > 0), "You should run train() and save model first"
        ...
        if len(all_models) == 1:
            ...
            for model_p in all_models:
                ...
                win_G_diff = np.abs(self.G1_losses[(niter-ws):(niter+ws)] + np.log(0.5))
