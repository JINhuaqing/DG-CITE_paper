...
params.prefix = ""
params.save_dir = f"simu_real_dataY0"
...
def _main_fn(rep_ix, params, lr, n_infeat, n_T, weight_decay, n_blk):
    ...
    if not ofil_name.exists():
        ...
        res_other.CQR = (prb_Y0_cqr, mlen_cqr)
        ...
        prb_Y0_cf = np.bitwise_and(((0-data_test.Y0)>CF_CIs[:, 0], (0-data_test.Y0)<CF_CIs[:, 1]).mean()
        res_other.CF= (prb_Y0_cf, mlen_cf)
