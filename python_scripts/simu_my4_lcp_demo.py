...
params.prefix = ""
params.save_dir = f"simuLCP_my4{setting}_c{args.c*100:.0f}_d{params.simu_setting.d}_trunc"
...
def _main_fn(rep_ix, params, lr, n_infeat, n_T, weight_decay, n_blk):
    ...
    cutoff1 = torch.quantile(test_Xnorm, 0.10)
    cutoff2 = torch.quantile(test_Xnorm, 0.90)
    ...
    cutoff1 = torch.quantile(val_Xnorm, 0.10)
    cutoff2 = torch.quantile(val_Xnorm, 0.90)
    val_X1 = val_X[val_Xnorm<cutoff1].clone()
    val_Y1 = val_Y[val_Xnorm<cutoff1].clone()
    val_X2 = val_X[val_Xnorm>cutoff2].clone()
    val_Y2 = val_Y[val_Xnorm>cutoff2].clone()
    val_X2c = val_X[val_Xnorm>=cutoff2].clone()
    val_Y2c = val_Y[val_Xnorm>=cutoff2].clone();
    ...
    if not fil_name.exists():
        ...
        res_all[f"LDDIM2"] = _inner_fn(test_X2, test_Y2, ddpm, gen_type="ddim", LCP=LCP)
        ...
        res_all[f"LDDIM2_val"] = _inner_fn(val_X2, val_Y2, ddpm, gen_type="ddim", LCP=LCP)
        ...
        res_all[f"LMLP2"] = _inner_fn(test_X2, test_Y2, net, gen_type="reg", LCP=LCP)
        ...
        res_all[f"LMLP2_val"] = _inner_fn(val_X2, val_Y2, net, gen_type="reg", LCP=LCP)
