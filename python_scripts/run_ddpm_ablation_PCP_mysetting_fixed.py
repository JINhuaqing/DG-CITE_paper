...
import argparse
parser = argparse.ArgumentParser(description='run ablation exps for Lei setting')
...
parser.add_argument('--early_stop', type=int, default=1, help='whether early stop or not') 
...
params.simu_setting.ntest = 2000
...
if params.ddpm_training.early_stop:
    params.save_dir = f"simu_ablation_my{setting}fixed_d{params.simu_setting.d}_n{params.simu_setting.n}"
else:
    params.save_dir = f"simu_ablation_my{setting}fixed_d{params.simu_setting.d}_n{params.simu_setting.n}_noearly"
...
def _run_fn_PCP(rep_ix, params, lr, n_infeat, n_T, weight_decay, n_blk):
    ...
    ddpm = myddpm.ddpm
    ddpm.eval()
    prbs1, mlen1 = _inner_fn(data_test1, ddpm)
    prbs1_val, mlen1_val = _inner_fn(data_val1, ddpm)
    ddpm = myddpm.get_opt_model()
    ddpm.eval()
    prbs2, mlen2 = _inner_fn(data_test1, ddpm)
    prbs2_val, mlen2_val = _inner_fn(data_val1, ddpm)
        
        
    # results from CQR
    CQR_CIs = get_CQR_CIs(X=data_train.X, Y=data_train.Y, 
                          T=data_train.T, Xtest=data_test1.X, 
                          nav=0, 
                          alpha=params.hypo_test.alpha, 
                          estimand="nonmissing",
                          fyx_est="quantBoosting", seed=manualSeed)
    mlen_cqr = np.median(CQR_CIs[:, 1] -  CQR_CIs[:, 0])
    prb_Y1_cqr = np.bitwise_and(data_test1.Y1>CQR_CIs[:, 0], data_test1.Y1<CQR_CIs[:, 1]).mean()
    prb_tau_cqr = np.bitwise_and(data_test1.tau>CQR_CIs[:, 0], data_test1.tau<CQR_CIs[:, 1]).mean()
