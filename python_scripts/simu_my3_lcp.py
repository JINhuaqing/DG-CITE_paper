...
params.prefix = ""
params.save_dir = f"simulocalLESS_my3{setting}_d{params.simu_setting.d}_n{params.simu_setting.n}"
...
def _main_fn(rep_ix, params, lr, n_infeat, n_T, weight_decay, n_blk):
    ...
    if not fil_name.exists():
        ...
        opt_h = get_opth(X=tmpX, hmin=0.1, local_method="RLCP", target_eff_size=int(tmpX.shape[0]/3))
        ...
        res_all.LDDIM_val = _inner_fn(val_X, val_Y, ddpm, gen_type="ddim", LCP=True)
        ...
        for model_ix in range(params.save_snapshot, params.ddpm_training.n_epoch, params.save_snapshot):
            ...
            res_all[f"LDDIM_ep{model_ix}_val"] = _inner_fn(val_X, val_Y, ddpm, gen_type="ddim", LCP=True)
